...
const getComparativa = async (req, res) => {
  try {
    ...
    FROM (
        ...
        SELECT DISTINCT codigo, subcodigo 
        FROM conteos 
        WHERE conteo_grupo_id IN (${idsArray.join(",")}) 
          AND estado = 'VIGENTE'
          AND empresa_id = :empresa_id
    ) AS productos_contados
    INNER JOIN productos p ON p.codigo = productos_contados.codigo 
        AND p.subcodigo = productos_contados.subcodigo 
        AND p.empresa_id = :empresa_id
    LEFT JOIN saldos_global sg ON p.codigo = sg.codigo 
        AND p.subcodigo = sg.subcodigo 
        AND p.empresa_id = sg.empresa_id
    LEFT JOIN conteos c ON p.codigo = c.codigo 
        AND p.subcodigo = c.subcodigo 
        AND c.conteo_grupo_id IN (${idsArray.join(",")})
    GROUP BY p.codigo, p.subcodigo, p.nombre, p.referencia, sg.saldo
